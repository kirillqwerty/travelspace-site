...
import { ArrowLeft, ArrowRight, BedDouble, Check, Clock, MapPin, Utensils, Waves } from "lucide-react";
...
export default function HotelPage() {
  ...
  const tourPath = `/tours/${hotel.tour_slug}`;
  const pricesPath = `${tourPath}#${hotel.tour_hotel_anchor}`;
  ...
  return <article className="section-container pb-12 pt-24 lg:pt-28" data-testid="hotel-page">
    ...
    <Link to={pricesPath} className="inline-flex items-center gap-2 rounded-full border border-neutral-200 px-4 py-2.5 text-sm text-neutral-600 transition hover:border-orange-200 hover:text-[#C2410C]" data-testid="hotel-return-to-tour" data-navigation-back="true"><ArrowLeft className="size-4 shrink-0" />Вернуться к туру</Link>
    ...
    <div className="mt-5 flex flex-wrap gap-3 lg:hidden"><Link to={pricesPath} className="inline-flex items-center gap-2 rounded-full bg-[#C2410C] px-5 py-2.5 text-sm font-medium text-white">Даты и цены<ArrowRight className="size-4" /></Link><Button variant="outline" className="rounded-full" onClick={() => setInquiry({})}>Задать вопрос</Button></div>
    <div className="mt-7 grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_320px] lg:gap-10">
      <div className="min-w-0 space-y-8">
        ...
        {!!rooms.length && <DetailSection id="hotel-rooms" title="Номера"><p className="mt-3 text-sm leading-6 text-neutral-500">Выберите подходящий вариант размещения. Стоимость и доступность зависят от даты поездки — актуальные предложения указаны в туре.</p><div className="mt-5 space-y-6">{rooms.map((room, index) => <div key={room.id || index} className="rounded-2xl border border-neutral-200 p-4 sm:p-5" data-testid="hotel-room"><h3 className="mb-4 flex items-center gap-2 text-lg font-semibold"><BedDouble className="size-5 shrink-0 text-[#C2410C]" />{room.title || room.number || `Номер ${index + 1}`}</h3><PhotoGallery images={room.gallery || []} alts={room.gallery_alts} name={room.title || "Номер отеля"} />{room.description && <RichText text={room.description} className="mt-4 text-sm leading-7 text-neutral-600" />}{getYoutubeVideoId(room.video_url) && <LazyYoutubeEmbed value={room.video_url} title={`Обзор: ${room.title || room.number || hotel.name}`} className="mt-4" />}<div className="mt-5 flex flex-wrap gap-3"><Link to={pricesPath} className="inline-flex items-center gap-2 rounded-full bg-[#C2410C] px-5 py-2.5 text-sm font-medium text-white hover:bg-[#9A3412]">Даты и стоимость<ArrowRight className="size-4" /></Link><button className="rounded-full border px-5 py-2.5 text-sm text-neutral-700 hover:bg-neutral-50" onClick={() => setInquiry(room)}>Уточнить о номере</button></div></div>)}</div></DetailSection>}
      ...
      <aside className="rounded-2xl border border-neutral-200 bg-stone-50 p-6 lg:sticky lg:top-28">
        <p className="text-xs uppercase tracking-wider text-neutral-500">Отель доступен в туре</p><h2 className="mt-3 text-xl font-semibold leading-snug">{hotel.tour_title}</h2>
        <div className="my-5 space-y-3 border-y border-neutral-200 py-5 text-sm text-neutral-600">{hotel.meal && <p className="flex gap-2"><Utensils className="size-4 shrink-0" />{hotel.meal}</p>}{hotel.location && <p className="flex gap-2"><MapPin className="size-4 shrink-0" />{hotel.location}</p>}{hotel.beach && <p className="flex gap-2"><Waves className="size-4 shrink-0" /><span>Информация о пляже — в описании</span></p>}<p className="leading-6">Программа поездки, даты и стоимость с проживанием собраны на странице тура.</p></div>
        <Link to={pricesPath} className="flex items-center justify-center gap-2 rounded-full bg-[#C2410C] px-5 py-3 text-sm font-medium text-white hover:bg-[#9A3412]">Даты и цены<ArrowRight className="size-4" /></Link>
        <Button variant="outline" className="mt-3 w-full rounded-full bg-white" onClick={() => setInquiry({})}>Задать вопрос об отеле</Button><Link to={tourPath} className="mt-4 block text-center text-sm text-neutral-500 hover:text-[#C2410C]">Посмотреть программу тура</Link>
      </aside>
    </div>
    {inquiry && <Suspense fallback={null}><LeadDialog open onOpenChange={(open) => { if (!open) setInquiry(null); }} tour={{ id: hotel.tour_id, slug: hotel.tour_slug, title: hotel.tour_title, dates: hotel.dates }} dates={hotel.dates} selectedHotel={hotel.name} selectedRoom={inquiry.title || inquiry.number || ""} title="Уточнить детали проживания" description="Оставьте контакты — менеджер поможет с выбором номера и дат поездки." /></Suspense>}
  </article>;
}
