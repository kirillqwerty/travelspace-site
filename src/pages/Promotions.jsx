...
export default function Promotions() {
  ...
  return (
    <div className="section-container section-pad" data-testid="promotions-page">
      <p className="overline text-[#C2410C]">Акции</p>
      <h1 className="font-heading text-4xl sm:text-5xl lg:text-6xl mt-3 max-w-3xl">Спецпредложения сезона</h1>

      <div className="mt-12 grid md:grid-cols-2 gap-6">
        {items.map((p) => (
          <div key={p.id} className="rounded-2xl overflow-hidden border border-neutral-200 bg-white flex flex-col">
            {p.image && (
            ...
            <div className="p-6 flex-1 flex flex-col">
              ...
              <p className="text-sm text-neutral-600 mt-3 leading-relaxed">{p.description}</p>
              {p.valid_until && (
                <p className="mt-4 text-xs text-neutral-500 inline-flex items-center gap-1">
                  <Calendar className="size-3.5" /> Действует до {p.valid_until}
                </p>
              )}
