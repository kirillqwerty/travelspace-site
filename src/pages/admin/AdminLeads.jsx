    ...
    <div data-testid="admin-leads">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-neutral-500 mt-1">Список всех заявок, оставленных через сайт.</p>
        </div>
      ...
      <div className="mt-8 rounded-2xl bg-white border border-neutral-200 overflow-x-auto">
        <table className="w-full text-sm min-w-[860px]">
          ...
          <tbody className="divide-y divide-neutral-100">
            {loading && (
              <tr>
                <td colSpan={8} className="px-5 py-10 text-center text-neutral-400">Загрузка…</td>
              </tr>
            )}
            {!loading && leads.length === 0 && (
              <tr>
                <td colSpan={8} className="px-5 py-10 text-center text-neutral-400">Пока нет заявок</td>
              </tr>
            )}
            {leads.map((l) => (
              <tr key={l.id} data-testid={`lead-row-${l.id}`}>
                <td className="px-5 py-3 text-neutral-500 whitespace-nowrap">{l.created_at?.slice(0, 16).replace("T", " ")}</td>
                <td className="px-5 py-3 text-xs">{l.form_type}</td>
                <td className="px-5 py-3">{l.name || "—"}</td>
                <td className="px-5 py-3 font-mono whitespace-nowrap">{l.phone}</td>
                <td className="px-5 py-3 max-w-[200px] truncate">{l.tour || l.region || "—"}</td>
                <td className="px-5 py-3 max-w-[260px] truncate text-neutral-600">{l.comment || ""}</td>
                <td className="px-5 py-3">
                  <Select value={l.status || "new"} onValueChange={(v) => onStatus(l.id, v)}>
                ...
                <td className="px-5 py-3">
                  <button onClick={() => onDelete(l.id)} className="text-neutral-400 hover:text-red-600" data-testid={`lead-delete-${l.id}`}>
