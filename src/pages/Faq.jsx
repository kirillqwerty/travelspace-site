      ...
      <div className="mt-12 space-y-10">
        {Object.entries(grouped).map(([cat, list]) => (
          <div key={cat}>
            ...
            <Accordion type="single" collapsible className="mt-3 border-y border-neutral-200">
              {list.map((q) => (
                <AccordionItem key={q.id} value={q.id} className="px-1">
                  <AccordionTrigger className="text-left text-base font-medium py-5">{q.question}</AccordionTrigger>
                  <AccordionContent className="text-neutral-700 leading-relaxed">{q.answer}</AccordionContent>
                </AccordionItem>
              ))}
            </Accordion>
          </div>
